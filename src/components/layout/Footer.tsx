    ...
    <footer className="bg-gray-800 text-white">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-wrap justify-between gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {[['/','nav.home'],['/blog','nav.blog'],['/categories','nav.categories'],['/contact','nav.contact']].map(([href, key]) => (
                <li key={href}><Link href={href} className="text-gray-400 hover:text-white">{t(key)}</Link></li>
              ))}
            </ul>
          </div>
          <div>
        ...
        <div className="mt-8 border-t border-gray-700 pt-8 text-center text-gray-400">
          <p>&copy; {new Date().getFullYear()} My Blog. {t('footer.rights')}</p>
        </div>
      </div>
    </footer>
  );
